...
# TCP distinguishes connections with a 4-tuple (src IP, src Port, dest IP, dest Port)
# As a result, an extra path (so an extra socket) == an extra active connection
# Maintains independent congestion control and failure isolation - AS AN APPLICATION LAYER element

# Server needs to know the connections belong to the SAME client.
# Same IP is not enough - it's a network interface. Every device on the home WiFi has the same IP (NAT)
# An application-level identity is required... like a session_id

# When a client connects, a unique session_id is generated by the SERVER and sent to the client.
# The client will include this session_id in EVERY frame sent.
# All sockets of a client use the SAME session_id

sessions = dict() #key is session_id, value is the items dict
...
def main():
    ...
    while True:
        ...
        if value == "NEW":
            ...
            sessions[session_id] = {}
            communication_socket.sendall(f"SESSION|{session_id}\n".encode("utf-8"))
            new_session_id += 1
        else:
